...
class DatabaseService {
  ...
  async initDatabase(): Promise<void> {
    try {
      ...
      await this.migrarSenhasParaHash();
    } catch (error) {
  ...
  async getClientes(): Promise<Cliente[]> {
    ...
    return querySnapshot.docs.map(doc => ({
      ...
      horarioEntrada: doc.data().horarioEntrada?.toDate?.()?.toISOString() || new Date().toISOString(),
  ...
  async atualizarConfiguracao(id: number, config: Partial<Configuracao>): Promise<void> {
  ...
  async inserirNotificacao(notificacao: Omit<Notificacao, 'id'>): Promise<string> {
  ...
  async verificarLogin(usuario: string, senha: string): Promise<{ sucesso: boolean; barbeiro?: string; nome?: string }> {
    try {
      const q = query(
  ...
  async migrarSenhasParaHash(): Promise<void> {
    try {
      ...
      for (const doc of snapshot.docs) {
        ...
        // Se a senha não está hasheada (não começa com $2b$ que é o padrão do bcrypt)
        if (userData.senha && !userData.senha.startsWith('$2b$')) {
          console.log(`Migrando senha de ${userData.usuario}...`);
          await updateDoc(doc.ref, {
            senha: await hashSenha(userData.senha),
            updatedAt: serverTimestamp()
          });
        }
      }
  ...
  async alterarSenha(usuario: string, senhaAtual: string, novaSenha: string): Promise<boolean> {
    try {
      const q = query(
        collection(this.db, 'usuarios'),
        where('usuario', '==', usuario.toLowerCase())
